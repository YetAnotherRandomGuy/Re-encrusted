...
const storyPath = 'newer/_TRINITY.DAT';
if (!fs.existsSync(storyPath)) {
  console.log('SKIP: newer/_TRINITY.DAT not present');
  process.exit(0);
}
...
try {
  ...
  console.error('FAIL: Trinity WASM trapped during initial load/step');
...
const out = printed();
if (/screen too narrow/i.test(out)) {
  console.error('FAIL: Trinity rejected interpreter screen width');
  console.error(out.slice(-1500));
  process.exit(1);
}
if (!/trinity|copyright|infocom|press any key|waking up|west of house/i.test(out)) {
  console.error('FAIL: expected Trinity startup/progress text');
  console.error(out.slice(-1500));
  process.exit(1);
}

console.log('V4 Trinity WASM smoke passed');
